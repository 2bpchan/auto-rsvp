...
- **Word size setting** - adjustable reader text size (20-400%; the base sizes were halved so the old 50% became the new 100% default, since that's the size people actually wanted)
...
- **Fixed pivot point, and markers showing it** - the focus letter previously jittered a few pixels sideways depending on how wide that particular letter was (a `flex-shrink` side effect), since the reader's whole premise is a fixed fixation point that doesn't need re-fixating. Also added a pair of small, subtle arrows above/below the focus letter's fixed position
