...
- **Enclosing punctuation in the margins** - when the current word is inside parentheses, brackets, braces, angle brackets, or asterisks, the wrapping characters are stripped from the word and shown large and color-coded (by type) in the left/right margins instead, nested outermost-first, so they don't clutter the word and are easy to notice in peripheral vision. Supports multiple simultaneous/nested levels
